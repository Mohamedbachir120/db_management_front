...
import {  useGetBddQuery,  useFetchLinkedAccessQuery, useLinkAccessMutation } from '../../../features/bdd/bdd' 
...
import { useParams } from 'react-router-dom';
 ...
 export default function LinkedBDAccess() {
  ...
  const {data,isLoading} = useGetBddQuery(parseInt(id!));
  const {currentData,isFetching,refetch} = useFetchLinkedAccessQuery(parseInt(id!));
    ...
    <div className='d-flex flex-row'>
      ...
      <div className='col-9 pb-5'>
        ...
        {!isFetching && !isLoading ?
          screen == "edit"? 
          (<div > 
              <label className='d-block'>Selectionner les access de  bdd { data?.name } : </label>
          ...
          (
          <div>
        <h2 className="text-green"><FontAwesomeIcon icon={faList} /> Bdd : {data?.name}</h2>

          <h5>Access de  bdd {data?.name} :</h5>
          {
            !isFetching ?
            (
              <div>
