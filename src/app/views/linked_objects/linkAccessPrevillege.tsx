...
import { useGetPrivillegeQuery ,  useFetchLinkedAccessQuery, useLinkAccessMutation} from '../../../features/privillege/privillege';



 export default function LinkedAccessPrevillege() {
    ...
    <div className='d-flex flex-row'>
      ...
      <div className='col-9 pb-5'>
        ...
        {!isFetching && !isLoading ?
          screen == "edit"? 
          (<div > 
              <label className='d-block'>Selectionner les access de  privillège { data?.name } : </label>
          ...
          (
          <div>
        <h2 className="text-green"><FontAwesomeIcon icon={faList} /> Privillège : {data?.name}</h2>

          <h5>Access de  privillège {data?.name} :</h5>
          {
            !isFetching ?
            (
              <div>
             {currentData?.map((e:Access)=>(
